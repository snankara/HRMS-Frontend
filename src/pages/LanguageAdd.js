...
export default function LanguageAdd(props) {
    ...
    const [languages, setLangugaes] = useState([{ languageName: "", level: "" }])
                ...
                return (
                    <div key={`${language}-${i}`}>
                        ...
                        <div className="btn-box">
                            {languages.length !== 1 && <Button className="btn-link mr10" color="danger"
                                onClick={() => handleRemoveClick(i, ...languages)}>Sil</Button>}

                            {languages.length - 1 === i && <Button className="btn-link" color="info" onClick={() => handleAddClick(...languages)}>
