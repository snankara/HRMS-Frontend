...
export default function TechnologyAdd(props) {
    ...
    const [technologies, setTechnologies] = useState([{ technologyName: "" }])
                    ...
                    <div key={`${technology}-${i}`}>
                        <Row>
                            ...
                            <div className="btn-box" style={{marginTop:'-8px'}}>
                                {technologies.length !== 1 && <Button className="btn-link mr10" color="danger"
                                    onClick={() => handleRemoveClick(i, ...technologies)}>Sil</Button>}

                                {technologies.length - 1 === i && <Button className="btn-link" color="info" onClick={() => handleAddClick(...technologies)}>
